...
struct Requests {
    ...
    static func getMoviesByGenre(genre: String, completionHandler: @escaping (Movie?) -> Void) -> Void {
        
        let request = URLRequest(url: BaseService.discoverMovieByGenre(genre: genre).path)
        URLSession.shared.dataTask(with: request, completionHandler: { (data, response, error) in
            if let error = error {
              print("Error with fetching films: \(error)")
              return
